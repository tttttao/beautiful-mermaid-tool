...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isSaved ? 'Chart saved' : 'Save Chart'" @click="handleSave">
      <span class="sr-only" aria-live="polite">{{ isSaved ? 'Chart saved' : 'Save Chart' }}</span>
      <Check v-if="isSaved" aria-hidden="true" class="h-5 w-5 text-emerald-500 dark:text-emerald-400 transition duration-200 scale-110" />
      <Save v-else aria-hidden="true" class="h-5 w-5 transition duration-200 group-hover:scale-110" />
    </button>
