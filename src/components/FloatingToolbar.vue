...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button
      ...
      :disabled="disabled"
      type="button"
      :title="isSaved ? 'Saved' : 'Save Chart'"
      :aria-label="isSaved ? 'Saved' : 'Save Chart'"
      aria-live="polite"
      @click="handleSave"
    >
      <Check v-if="isSaved" class="h-5 w-5 transition duration-200 text-emerald-500 dark:text-emerald-400 scale-110" />
