...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="showSaveFeedback ? 'Saved!' : 'Save Chart'" :aria-label="showSaveFeedback ? 'Saved!' : 'Save Chart'" @click="handleSave">
      <Check v-if="showSaveFeedback" class="h-5 w-5 transition duration-200 text-emerald-500" />
