...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isSaving ? 'Saved!' : 'Save Chart'" :aria-label="isSaving ? 'Saved!' : 'Save Chart'" @click="handleSave" aria-live="polite">
      <Check v-if="isSaving" class="h-5 w-5 text-emerald-500 transition duration-200" />
