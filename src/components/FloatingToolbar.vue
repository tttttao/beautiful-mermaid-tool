...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]" aria-live="polite">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isSaved ? 'Saved!' : 'Save Chart'" :aria-label="isSaved ? 'Saved!' : 'Save Chart'" @click="handleSave">
      <Check v-if="isSaved" class="h-5 w-5 text-emerald-500 transition duration-200" />
