...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isSaved ? 'Chart Saved' : 'Save Chart'" :aria-label="isSaved ? 'Chart Saved' : 'Save Chart'" @click="handleSave">
      <Check v-if="isSaved" class="h-5 w-5 text-emerald-600 dark:text-emerald-400 transition duration-200 group-hover:scale-110" />
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isExported ? 'PNG Exported' : 'Export PNG'" :aria-label="isExported ? 'PNG Exported' : 'Export PNG'" @click="handleExport">
      <Check v-if="isExported" class="h-5 w-5 text-emerald-600 dark:text-emerald-400 transition duration-200 group-hover:translate-y-0.5" />
