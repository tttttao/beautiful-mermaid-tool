...
function handleSave() {
  emit('save')
  isSaved.value = true
  feedbackMessage.value = 'Chart saved to history'
  clearTimeout(saveTimeout)
  saveTimeout = setTimeout(() => {
    isSaved.value = false
    if (feedbackMessage.value === 'Chart saved to history') {
...
function handleExport() {
  ...
  feedbackMessage.value = 'Chart exported to PNG'
  clearTimeout(exportTimeout)
  exportTimeout = setTimeout(() => {
    isExported.value = false
    if (feedbackMessage.value === 'Chart exported to PNG') {
      feedbackMessage.value = ''
    }
  }, 2000)
}
...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" title="Export PNG" aria-label="Export PNG" @click="handleExport">
      <Check v-if="isExported" class="h-5 w-5 text-emerald-500 transition duration-200" />
