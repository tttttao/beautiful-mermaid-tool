...
<template>
  <div class="absolute bottom-6 right-6 z-20 flex flex-wrap items-center justify-end gap-3 max-w-[calc(100%-3rem)]">
    ...
    <button :class="buttonClass" :disabled="disabled" type="button" :title="isSaved ? 'Chart saved' : 'Save Chart'" :aria-label="isSaved ? 'Chart saved' : 'Save Chart'" @click="handleSave">
      <Check v-if="isSaved" class="h-5 w-5 transition duration-200 scale-110 text-emerald-500" aria-hidden="true" />
      <Save v-else class="h-5 w-5 transition duration-200 group-hover:scale-110" aria-hidden="true" />
    </button>

    <div class="w-[1px] h-8 bg-slate-200 dark:bg-slate-700 mx-1" aria-hidden="true"></div>
