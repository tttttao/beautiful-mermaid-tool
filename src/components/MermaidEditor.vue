    ...
    <header class="flex items-center justify-between border-b border-slate-100 dark:border-white/10 px-6 py-4">
      ...
      <div class="flex items-center gap-3">
        <button
          ...
          class="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-300"
          @click="copy(modelValue)"
        >
          <Check v-if="copied" class="h-4 w-4 text-emerald-500" />
          <Copy v-else class="h-4 w-4" />
        </button>
        <span aria-live="polite" class="sr-only">{{ copied ? 'Copied source to clipboard' : '' }}</span>
