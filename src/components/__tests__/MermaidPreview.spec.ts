...
describe('FloatingToolbar component', () => {
  it('should emit all toolbar events', async () => {
    ...
    await buttons[2].trigger('click') // toggleSidebar
    ...
    expect(wrapper.emitted('copySource')).toHaveLength(1)
  ...
  it('should disable buttons when disabled prop is true', async () => {
    ...
    expect(buttons[0].attributes('disabled')).toBeUndefined() // toggleTheme
    expect(buttons[1].attributes('disabled')).toBeUndefined() // copy
    expect(buttons[2].attributes('disabled')).toBeUndefined() // history
